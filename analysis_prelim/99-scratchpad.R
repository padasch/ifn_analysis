...
df_tre <- df_tre |> group_by(idp, a) |> nest()
df_loc <- df_loc |> group_by(idp, lat, lon, dep, ser) |> nest()

# Attach coordinates to df_tre
df_tre <- 
  df_tre |> 
  left_join(
    df_loc |> 
...
# Attach variable stating number of visits of a tree
df_tre$n_visits <- NA
for (i in 1:nrow(df_tre)) {
  cat("\014", round(i / nrow(df_tre)*100, 4))
  df_tre$n_visits[i] <- nrow(df_tre$data[[i]])
}

# Save big dataframe (loop takes ~2h to run)
saveRDS(df_tre, here::here("data/tmp/df_tre-after-n_visits-loop.rds"))
df_tre <- readRDS(here::here("data/tmp/df_tre-after-n_visits-loop.rds"))

# Filter for trees that have two visits
df_visit <- df_tre |> filter(n_visits > 1) |> select(-n_visits)

# Attach information on visit by hand (error prone!)
# TODO How to remove error-proneness? Check each site?
df_visit <- df_visit |> unnest() |> ungroup()
df_visit$visit <- rep(c("v1", "v2"), nrow(df_visit)/2)

# Get variables for making wide df
l_names <- 
  df_visit |> 
  select(-visit, -idp, -a, -lat, -lon, -dep, -ser) |>
  names()
...
df_visit <- 
  ...
  pivot_wider(names_from = visit, 
              values_from = any_of(l_names))

# Unnest and remove columns with NA only
df_visit <- df_visit |> select(where(~!all(is.na(.)))) 

# What re-visit tree stati do we have?
df_visit$veget5_v2 |> table()
df_visit$dep |> table()

# Take subsets for healthy trees
df_alive <- df_visit |> filter(veget5_v2 == 0)
df_alive <- 
  df_alive |>
  mutate(delta_c13 = c13_v2 - c13_v1) |> 
  drop_na(delta_c13)

p_a1 <- 
  df_alive |> 
  ggplot() +
  aes(x = as.factor(campagne_v2), 
      y = delta_c13) +
  geom_violin(trim = FALSE) +
  geom_boxplot(width = 0.05) +
  theme_classic() +
  labs(x = "Year",
       y = "Circumference Increment [m]",
       title = "Circumference Increment at 1.30m")

df_alive2 <- 
  df_alive |> 
  select(campagne_v2, delta_c13) |> 
  group_by(campagne_v2) |> 
  summarise_all(list(mean = mean, se = std_error))

p_a2 <- 
  df_alive2 |> 
  ggplot() +
  geom_pointrange(aes(x = campagne_v2, y = mean, 
                      ymin = mean-se, ymax = mean+se)) +
  geom_line(aes(x = campagne_v2, y = mean)) +
  theme_classic() +
  labs(x = "Year",
       y = "Circumference Increment [m]")
...
# Take subsets for trees that died
df_dead <- 
  df_visit |> 
  filter(veget5_v2 %in% c("2", "5", "C", "M", "T"))

p_d1 <- 
  df_dead |> 
  ggplot() +
  aes(x = as.factor(campagne_v2), y = c13_v1) +
  geom_violin(trim = FALSE) +
  geom_boxplot(width = 0.05) +
  theme_classic() +
  labs(x = "Year",
       y = "Mean Volume [m^3]",
       title = "Dead Tree Volume")

df_dead2 <- 
  df_dead |> 
  select(campagne_v2, c13_v1) |> 
  group_by(campagne_v2) |> 
  summarise_all(.f = list(mean = mean, se = std_error), na.rm = TRUE)
...
# Take subsets for trees that were removed
df_rm <- 
  df_visit |> 
  filter(veget5_v2 %in% c("6"))
