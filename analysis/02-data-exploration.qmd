...
```{r}
datatable(l_metadata$metadata |> filter(tolower(variable) %in% names(l_raw_data$arbre)))
```
...
-   BUT not all trees are that small! Some are just above the DBH threshold of 0.24m, some are much bigger (c13 \> 1). This should not be the case for newly recruited trees?
...
df_loc$v2 |> table() |> 
  barplot(main = "Count of sites that were visited once but recorded as second visit",
          xlab  = "Year of '2nd Visit'")
df_tre$c13 |> hist(main = "Histogram of DBH measurements", breaks = 50, xlab = "DBH [m]")
```
...
Most trees are rather small with a DBH of ca. 0.2-0.4m, which could indicate that these are newly recruited trees, and that the respective sites are turning into forests. Nonetheless, there are quite a few large trees that were assessed the first time during the second visit, which is odd.
