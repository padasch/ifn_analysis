...
# Load the shapefile
hexmap <-
  st_read(here("data/raw/maps/hexmaps/qgis-hexmap-y_axis-75km.shp")) |>
  mutate(polyg_id = row_number()) |>
  select(geometry, polyg_id)
...
all_deps      <- hexmap |> pull(polyg_id) |> unique()
dummy         <- c(2015:2021)
...
p <- 
  df_plot %>%
  # filter(str_detect(census_interval, "2016")) |>  # DEBUG------------------
  ggplot(aes(fill = ba_growth_rate)) +
  geom_sf(color = "black") +
  ...
  scale_fill_viridis_c(
    ...
    # trans    = "log",
    na.value = "grey90",
    name     = "basal area loss [m2 trees / hectare land / yr]: \n"
    ) +
  # scale_fill_viridis(
  #   option = "B",
  #   trans = "log",
