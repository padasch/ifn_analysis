...
### Merge datastructure for newly sampled trees
```{r, eval=FALSE}
# Correction for newly grown trees. Above, their state is assigned to their first
# measurement but that occured during the second visit. To keep data consistent,
...
# - Overwrite veget with veget5 (veget is all NA for newly sampled trees)
# df_split_new$veget5 <- df_split_new$veget
# df_split_new$veget  <- NA
...
df_comb_qc <- 
  df_comb_size |> 
  mutate(c13_change = ( c13_2 - c13_1 ) / c13_1 / 5 ,
...
ggsave(
  paste0(
    "figures/", format(Sys.time(), format = "%Y-%d-%m_%H%M%S"),
    "_plot_",
...
df_filtered <- df_filtered |> 
  mutate(change = ifelse(change > 0, change, 0))
...
xxx |> 
  ...
  aes(y = growth_ba_yr_ha_scaled, 
      ...
      group = 1,
      fill = as.factor(campagne_1)) +
  # Add layout
  geom_abline(intercept = 0, slope = 1, linetype = "dotted") +
  facet_wrap(~dep) + 
  
  # Add arrow paths
  geom_path(aes(alpha = campagne_1),
