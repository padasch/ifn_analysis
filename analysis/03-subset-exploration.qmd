...
```{r}
df_comb_filtered <- 
  df_comb_updated |> filter(
    # Remove trees that have not been revisited
    revisit_state != "not_revisited", 
    
    # Remove trees that have no first circumference measurement
    !is.na(c13_1)
)
```
...
### Calculation of biomass changes

- deltas:
  -   For dead trees
    -   If they have a second c13 measurement, take that as negative
    -   If they have no second c13 measurement, take first measurement as negative
  - For cut trees
    -   If they have no second c13 measurement, take first measurement as negative
  - For alive trees
    -   Take second minus first measurement
