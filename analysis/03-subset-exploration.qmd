...
df_comb_tree_state <- 
  ...
  mutate(
    tmp = paste0(tree_state_1, "_", tree_state_2),
    tree_state_change = NA,
    tree_state_change = ifelse(tmp == "alive_alive", "survived", tree_state_change),
    tree_state_change = ifelse(tmp == "alive_dead", "died", tree_state_change)
  ) |> 
  select(-tmp)
```
...
df_filtered_trees <-
  ...
  filter(
    campagne_1 > 2009,
    tree_state_change %in% c("survived", "died"),
    cible %in% c(1)
    ) 
```
...
df_fin <- load_or_save_latest_file(nfi_dataset_for_analysis, "load")
```
...
```{r}
# Get most dominant species per dep as orientation
df_dominant_species <-  
  df_comb_qc |> 
  group_by(dep) |> 
  nest() |> 
...
### Loop to save df_plot()
```{r}
my_files <- list()

for (my_species in my_species_list) {
  
  cat("\n", my_species)
  
  # Filter for species
  df_species <- df_comb_qc |> filter(genus_lat == my_species)

  # QC
  if (nrow(df_species) == 0) stop("Empty df!")

  for (my_polygon in my_polygons_list) {
    
    # Settings for hexmaps
    n_polygons_per_degree  <- my_polygon
    ...
    my_filename <- 
      paste0(
        ...
        "hexmap_data-",
        my_species, "-",
        n_polygons_per_degree, "polygons_per_degree-",
        min_ntrees_per_polygon, "min_ntrees_per_polygon",
        ".rds"
        )
    
    my_files <-  append(my_files, my_filename)
   ...
   # Get data     
    ...
    # Save data
    saveRDS(df_plot, my_filename)
  }
}
```

```{r}
df_plot <- readRDS(here("data/tmp/hexmap_data-Quercus-30polygons_per_degree-5min_ntrees_per_polygon.rds"))
create_hexmap_from_aggregated_data("n_mor_yr", df_plot, "return") + theme_classic()
```

## Other comparisons
TODO:

- Take care of outlier in ridges without limits

```{r}
df_species |>
  drop_na(c13_change_perc_yr, height_class) |>
  ggplot() +
  geom_density_ridges(aes(
    x = c13_change_perc_yr,
...
df_species |>
  drop_na(c13_change_perc_yr, age_class) |>
  mutate(campagne_1 = fct_rev(as.factor(campagne_1))) |> 
  ggplot() +
  aes(
    x = c13_change_perc_yr,
    y = campagne_1,
