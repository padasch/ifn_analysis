...
df_forestconditions <- df_tmp |> dplyr::select(-data) |> right_join(df_fin_human, by = join_by(idp))
toc()
beepr::beep()
```
...
# Quality check, no new rows should be added
if (nrow(df_after_adding_vars) != nrow(df_tmp)) {
  beepr::beep(5)
  error("MERGING DUPLICATED SITES, SHOULD NOT HAPPEN!")
}
