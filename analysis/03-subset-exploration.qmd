...
```{r}
all_species <- get_most_common_species(df, "genus")
df_tmp <- df |> filter(genus_lat == "Quercus")
df_plot <- calculate_growth_mortality_per_area(df_species  = df_tmp, region_type = "se")
create_hexmap_from_aggregated_data(
  selected_var = "ba_loss_rate",
  df_plot  = df_plot,
  return_or_save = "return",
  selected_species = "Populus"
  )
```
