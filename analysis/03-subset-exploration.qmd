...
df_luc_final <- 
  left_join(
    df_comb_size,
    df_change,
    by = join_by(idp, uta1, uta2, autut_1, autut_2, utip_1, utip_2, csa_1, csa_2)
  )
...
# ______________________________________________________________________________
# Final df
df_after_apply <- df_fin_human
```

```{r}
columns_only_in_new_df <- setdiff(names(df_after_apply), names(df_before_apply))
columns_only_in_final_df <- setdiff(names(df_before_apply), names(df_after_apply))
columns_not_in_both <- base::union(columns_only_in_new_df,
                                   columns_only_in_final_df)

# Print or use the result as needed
message(
  "> New variables added to dataset: \n  ",
