...
# Time of interest
toi <- gee_get_time_of_interest("3months")

# Dataset of interest
my_data  <- "ECMWF/ERA5_LAND/DAILY"   # ERA5 Daily
...
if (my_data == "ECMWF/ERA5_LAND/DAILY") {
...
# LANDSAT-8
# TODO: Can also be used to calculate NDVI and EVI!
```
...
for (i in 2:length(toi) - 1) {
  ...
  for (my_var in voi) {
      ...
      # TODO Split this section into multiple functions like
      # - create_era5_task
      # - create_modis_task
      # - create_s2_task
      # - create_l8_tast
      ...
      if (function_to_apply == "mean")   image <- image$mean()
      if (function_to_apply == "median") image <- image$median()
      if (function_to_apply == "max")    image <- image$max()
      if (function_to_apply == "min")    image <- image$min()
      
      # TODO Resample to 500m if given at coarser resolution
